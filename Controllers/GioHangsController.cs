...
namespace QuanLyFastFood.Controllers
{
    public class GioHangsController : Controller
    {
        ...
        public ActionResult CheckOutProduct(int makh, string tenkh, string diachi, string SoDienThoai, string email, string diachigiaohang, string checkGiaoHang, string httt, string voucher)
        {
            List<GioHang> gh = (List<GioHang>)Session[giohang];
            ...
            dh.ThanhToan = httt;
            //**check voucher
            Voucher cvou = db.Vouchers.Find(voucher);
            if(voucher.Length > 0)
            {
                if (cvou != null)
                {
                    dh.ApDungVoucher = cvou.MaVoucher;
                }
                else
                {
                    ViewBag.failedVoucher = "Voucher này không tồn tại hoặc đã hết hạn sử dụng";
